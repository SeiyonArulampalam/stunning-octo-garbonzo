...
void solve(cusparseHandle_t handle, cusparseStatus_t status, int mb, int nnzb,
...
int main() {
  ...
  // Call solve function
  solve(handle, status, mb, nnzb, blockDim, d_bsrVal, d_bsrRowPtr, d_bsrColInd,
        d_x, d_y, d_z);

  // Send the solution back to the cpu
